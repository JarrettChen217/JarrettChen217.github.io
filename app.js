...
function detailSections(p){
 const sections={
  ...
  product:()=>gallery(p,'product'),contributions:()=>p.work.length?`<div class="detail-section"><h3>${label('Work & contribution','工作与贡献')}</h3><ul>${p.work.map(w=>`<li>${esc(t(w))}</li>`).join('')}</ul></div>`:'',
  demo:()=>demoVideo(p),journey:()=>journey(p),architecture:()=>architecture(p),engineering:()=>gallery(p,'engineering'),team:()=>gallery(p,'team'),credits:()=>credits(p),
 ...
 const order=p.sectionOrder||(p.process?['background','process','resources','scope']:['background','contributions','product','demo','journey','architecture','engineering','team','resources','scope']);
