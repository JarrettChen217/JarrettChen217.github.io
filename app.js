...
function featuredVideo(p){const video=p.featuredVideo;if(!video)return '';return `<section class="detail-section project-featured-video"><h3>${label('Gameplay demo','游戏演示')}</h3><a class="video-poster" href="${esc(video.watchUrl)}" target="_blank" rel="noopener" aria-label="${label('Watch gameplay demo on YouTube','前往 YouTube 观看游戏演示')}"><img src="${esc(video.poster)}" width="${video.width}" height="${video.height}" alt="${esc(t(video.caption))}" decoding="async"><span aria-hidden="true"><b>▶</b> ${label('Watch Demo','观看演示')}</span></a><p class="media-caption">${esc(t(video.caption))}</p><div class="project-actions"><a class="project-action" href="${esc(video.watchUrl)}" target="_blank" rel="noopener">${label('Watch Demo','观看演示')}</a>${p.play?`<a class="project-action project-action-primary" href="${esc(p.play.url)}" target="_blank" rel="noopener">${esc(t(p.play.label))}</a>`:''}</div></section>`;}
